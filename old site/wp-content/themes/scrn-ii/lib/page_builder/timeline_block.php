...
/* Aqua Timeline Block */
if(!class_exists('AQ_Timeline_Block')) {
	class AQ_Timeline_Block extends AQ_Block {
		...
		function __construct() {
			...
			//create the widget
			parent::__construct('AQ_Timeline_Block', $block_options);
			
			//add ajax functions
			add_action('wp_ajax_aq_block_event_add_new', array($this, 'add_event'));
		...
		function form($instance) {
			...
			<div class="description cf">
				<ul id="aq-sortable-list-<?php echo $block_id ?>" class="aq-sortable-list" rel="<?php echo $block_id ?>">
					<?php
					$events = is_array($events) ? $events : $defaults['events'];
					$count = 1;
				...
				<p></p>
				<a href="#" rel="event" class="aq-sortable-add-new button">Add New</a>
				<p></p>
			</div>
			<?php
		}
		
		function event($event = array(), $count = 0) {
				global $block_id;
				echo '
			...
			?>
			<li id="<?php echo $this->get_field_id('events') ?>-sortable-item-<?php echo $count ?>" class="sortable-item" rel="<?php echo $count ?>">
				
				<div class="sortable-head cf">
					<div class="sortable-title">
						<strong>Timeline Event <?php echo $count;?></strong>
					</div>
				...
				<div class="sortable-body">
					<p class="event-percent description">
						<label for="<?php echo $this->get_field_id('events') ?>-<?php echo $count ?>-percent">
							Timeline Event Position(0 - 100 values, difference of at least 5 between events)<br/>
							<input type="text" id="<?php echo $this->get_field_id('events') ?>-<?php echo $count ?>-percent" class="input-full" name="<?php echo $this->get_field_name('events') ?>[<?php echo $count ?>][percent]" value="<?php echo $event['percent'] ?>" />
						</label>
					</p>
					
					<p class="event-text description half">
						<label for="<?php echo $this->get_field_id('events') ?>-<?php echo $count ?>-text">
							Event title(what happened at that time)<br/>
							<input type="text" id="<?php echo $this->get_field_id('events') ?>-<?php echo $count ?>-text" class="input-full" name="<?php echo $this->get_field_name('events') ?>[<?php echo $count ?>][text]" value="<?php echo $event['text'] ?>" />
						</label>
					</p>

					<p class="event-when description half last">
						<label for="<?php echo $this->get_field_id('events') ?>-<?php echo $count ?>-when">
							Event date(year or month for example)<br/>
							<input type="text" id="<?php echo $this->get_field_id('events') ?>-<?php echo $count ?>-when" class="input-full" name="<?php echo $this->get_field_name('events') ?>[<?php echo $count ?>][when]" value="<?php echo $event['when'] ?>" />
						</label>
					</p>

					<p class="event-desc description"><a href="#" class="sortable-delete">Delete</a></p>
				</div>
		...
		function block($instance) {
			...
			echo '<ol class="lens-profile-timeline">';
					
			foreach( $events as $event ){
				$percent = (int)$event['percent'];
				echo '<li style="left: ' . $percent . '%">';

				if($event['when'] != '') {
					echo '<time>' . esc_attr($event['when']) . '</time>';
				}

				if($event['text'] != '') {
					echo '<p>' . esc_attr($event['text']) . '</p>';
				}
			}
