...
class AQ_Team_Block extends AQ_Block {
	...
	function form($instance) {
		...
		$options = array('1' => 'Avatar on the right', '2' => 'Avatar on the left');
		
		?>
		<p class="description half">
			<label for="<?php echo $this->get_field_id('tname') ?>">
				Name
				<?php echo aq_field_input('tname', $block_id, $tname, $size = 'full') ?>
			</label>
		</p>
		...
		<p class="description half last">
			<label for="<?php echo $this->get_field_id('style') ?>">
				Image Style:
				<?php echo aq_field_select('style', $block_id, $options, $style); ?>
			</label>
		</p>
		...
		<p class="description half last">
			<label for="<?php echo $this->get_field_id('dribbble') ?>">
				Dribbble URL<br/>
				<?php echo aq_field_input('dribbble', $block_id, $dribbble, $size = 'full') ?>
			</label>
		</p>
		...
		<p class="description half">
			<label for="<?php echo $this->get_field_id('instagram') ?>">
				Instagram URL
				<?php echo aq_field_input('instagram', $block_id, $instagram, $size = 'full') ?>
			</label>
		</p>
	...
	function block($instance) {
		...
		if($style == '1') {
			...
			echo '<h4 class="text-right">' . esc_attr($tname) . '</h4>';
			echo '<p class="text-right">' . esc_attr($description) . ' </p>';
			
			echo '<ul class="list-unstyled list-inline text-right">';
			if($facebook != '') {
		       echo '<li><a target="_blank" rel="nofollow" href="' . esc_url($facebook) . '"><i class="icon fa fa-facebook"></i></a></li>';
		    }
			if($twitter != '') {
		        echo '<li><a target="_blank" rel="nofollow" href="' . esc_url($twitter) . '"><i class="icon fa fa-twitter"></i></a></li>';
		    }
		    if($linkedin != '') {
		        echo '<li><a target="_blank" rel="nofollow" href="' . esc_url($linkedin) . '"><i class="icon fa fa-linkedin"></i></a></li>';
		    }
		    if($pinterest != '') {
		        echo '<li><a target="_blank" rel="nofollow" href="' . esc_url($pinterest) . '"><i class="icon fa fa-pinterest"></i></a></li>';
		    }
		    if($dribbble != '') {
		    	echo '<li><a target="_blank" rel="nofollow" href="' . esc_url($dribbble) . '"><i class="icon fa fa-dribbble"></i></a></li>';
		    }
		    if($instagram != '') {
		    	echo '<li><a target="_blank" rel="nofollow" href="' . esc_url($instagram) . '"><i class="icon fa fa-instagram"></i></a></li>';
		    }
		    if($skype != '') {
		    	echo '<li><a target="_blank" rel="nofollow" href="skype:' . esc_attr($skype) . '"><i class="icon fa fa-skype"></i></a></li>';
		    }
		    echo '</ul>';
		    echo '</div>';
		    ...
		    if($image != '') {
		    	...
		    	echo '<img class="img-responsive" alt="' . esc_attr($tname) . '" src="' . esc_url($image) . '" />';
		    	echo '</div>';
		    }
		}
		else {
