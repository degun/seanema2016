...
if(!class_exists('AQ_FilterablePortfolio_Block')) {
	...
	function block($instance) {
		...
		$output = '<div class="show-me">' . __('show me:', 'teo') . '</div>';

		$output .= '<ul class="filter-list list-inline list-unstyled">
			<li class="filter" data-filter="all">' . __('all', 'teo') . '</li>';
		foreach($categories as $cat) {
			$cat_details = get_category($cat);
			$output .= '<li class="filter" data-filter=".' . $cat_details->slug . '">' . $cat_details->name . '</li>';
		}
		$output .= '</ul>';

		$output .= '<div id="work-container' . $id . '" class="work-container">';

		$output .= '<ul class="work-grid">';
		$args = array();
		...
		while($query->have_posts() ) : $query->the_post(); global $post;
			...
			$output .= '<li class="mix ' . $class . '">
					<a href="' . get_permalink() . '">
						<img src="' . teo_resize($thumbnail, 428, 300) . '" alt="' . $title . '" />
						<div><h3>' . __('view project', 'teo') . '</h3></div>
					</a>
				</li>';
