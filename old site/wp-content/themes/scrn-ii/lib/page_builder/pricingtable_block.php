...
class AQ_PricingTable_Block extends AQ_Block {
	...
	function __construct() {
		...
		//add ajax functions
		add_action('wp_ajax_aq_block_feature_add_new', array($this, 'add_feature'));
	...
	function form($instance) {
		...
		$defaults = array(
			...
			'background'				=> '#787878',
			'color_scheme'				=> 1,
			'background_button' 		=> '#787878',
			'color_button'				=> '#ffffff',
			'background_button_hover' 	=> '#ff9172',
		...
		<p class="description half last">
			<label for="<?php echo $this->get_field_id('period') ?>">
				Price period(e.g. /mo, /yr):
				<?php echo aq_field_input('period', $block_id, $period, $size = 'full') ?>
			</label>
		</p>
		...
		<p class="description half">
			<label for="<?php echo $this->get_field_id('background') ?>">
				Background color
				<?php echo aq_field_color_picker('background', $block_id, $background, '#787878') ?>
			</label>
		</p>

		<p class="description half last">
			<label for="<?php echo $this->get_field_id('color_scheme') ?>">
				Color scheme for the text<br/>
				<?php echo aq_field_select('color_scheme', $block_id, $color_schemes, $color_scheme) ?>
			</label>
		</p>
		...
		<div class="description cf">
			<ul id="aq-sortable-list-<?php echo $block_id ?>" class="aq-sortable-list" rel="<?php echo $block_id ?>">
				<?php
				$features = is_array($features) ? $features : $defaults['features'];
				$count = 1;
				foreach($features as $feature) {	
					$this->feature($feature, $count);
					$count++;
				}
				?>
			</ul>
			<p></p>
			<a href="#" rel="feature" class="aq-sortable-add-new button">Add new feature</a>
			<p></p>
		</div>
		...
		<p class="description half">
			<label for="<?php echo $this->get_field_id('background_button') ?>">
				Button Background color
				<?php echo aq_field_color_picker('background_button', $block_id, $background_button, '#ffffff') ?>
			</label>
		</p>

		<p class="description half last">
			<label for="<?php echo $this->get_field_id('color_button') ?>">
				Button Text Color
				<?php echo aq_field_color_picker('color_button', $block_id, $color_button, '#3a3a3a') ?>
			</label>
		</p>

		<p class="description half">
			<label for="<?php echo $this->get_field_id('background_button_hover') ?>">
				Button Background hover color
				<?php echo aq_field_color_picker('background_button_hover', $block_id, $background_button_hover, '#ff9172') ?>
			</label>
		</p>

		<p class="description half last">
			<label for="<?php echo $this->get_field_id('color_button_hover') ?>">
				Button Text hover color
				<?php echo aq_field_color_picker('color_button_hover', $block_id, $color_button_hover, '#ffffff') ?>
			</label>
		</p>
	...
	function feature($feature = array(), $count = 0) {
			
		?>
		<li id="<?php echo $this->get_field_id('features') ?>-sortable-item-<?php echo $count ?>" class="sortable-item" rel="<?php echo $count ?>">
				
			<div class="sortable-head cf">
				<div class="sortable-title">
					<strong>Feature <?php echo $count;?></strong>
				</div>
			...
			<div class="sortable-body">
				<p class="tab-desc description">
					<label for="<?php echo $this->get_field_id('features') ?>-<?php echo $count ?>-content">
						Feature name<br/>
						<input id="<?php echo $this->get_field_id('features') ?>-<?php echo $count ?>-content" class="textarea-full" name="<?php echo $this->get_field_name('features') ?>[<?php echo $count ?>][content]" value="<?php echo $feature['content'] ?>" />
					</label>
				</p>
	...
	function block($instance) {
		...
		if($background != '') {
			$style = ' style="background-color: ' . esc_attr($background) . '" ';
		}

		echo '<div ' . $style . ' class="pricing-table ' . $class . '">';

		if($title2 != '') {
			echo '<p class="pt-title">' . esc_attr($title2) . '</p>';
		}

		if($price != '') {
			if($period != '') {
				echo '<p class="pt-price">' . esc_attr($price) . '<span>' . esc_attr($period) . '</span></p>';
			}
			else {
				echo '<p class="pt-price">' . esc_attr($price) . '</p>';
			}
		}
		...
		if($button_text != '') {
			if($button_url != '') {
				echo '<a target="_blank" id="btn-' . $id . '" class="btn btn-default" href="' . esc_url($button_url) . '">' . esc_attr($button_text) . '</a>';
			}
			else {
				echo '<a target="_blank" id="btn-' . $id . '" class="btn btn-default">' . esc_attr($button_text) . '</a>';
			}

			echo '<style type="text/css">
					#btn-' . $id . ' { background-color: ' . $background_button . '; color: ' . $color_button . '; border-color: ' . $background_button_hover . '; }
					#btn-' . $id . ':hover { background-color: ' . $background_button_hover . '; color: ' . $color_button_hover . '; }
				</style>';
		}
	...
	function add_feature() {
		...
		//default key/value for the feature
		$feature = array(
			'content' => 'Feature name'
		);
