...
/* Aqua Tabs Block */
class AQ_CustomButton_Block extends AQ_Block {
	...
	function __construct() {
		...
		//create the widget
		parent::__construct('AQ_CustomButton_Block', $block_options);
	...
	function form($instance) {
		...
		$defaults = array(
			...
			'background' => '#ffffff',
			'color'		=> '#3a3a3a',
			'background_hover' => '#ff9172',
			'color_hover'	=> '#ffffff'
		);
		$instance = wp_parse_args($instance, $defaults);
		extract($instance);
		$options = array("no" => "no", "yes" => "yes");
		$extra_border 	= array(1 => "No", 2 => "Yes");
		?>
		...
		<p class="description half last">
			<label for="<?php echo $this->get_field_id('extra_border') ?>">
				Extra border for the button?
				<?php echo aq_field_select('extra_border', $block_id, $extra_border, $extra_border) ?>
			</label>
		</p>
		...
		<p class="description half">
			<label for="<?php echo $this->get_field_id('background') ?>">
				Background color
				<?php echo aq_field_color_picker('background', $block_id, $background, '#ffffff') ?>
			</label>
		</p>
		...
		<p class="description half last">
			<label for="<?php echo $this->get_field_id('color') ?>">
				Text Color
				<?php echo aq_field_color_picker('color', $block_id, $color, '#3a3a3a') ?>
			</label>
		</p>

		<p class="description half">
			<label for="<?php echo $this->get_field_id('background_hover') ?>">
				Background hover color
				<?php echo aq_field_color_picker('background_hover', $block_id, $background_hover, '#ff9172') ?>
			</label>
		</p>

		<p class="description half last">
			<label for="<?php echo $this->get_field_id('color_hover') ?>">
				Text hover color
				<?php echo aq_field_color_picker('color_hover', $block_id, $color_hover, '#ffffff') ?>
			</label>
		</p>
	...
	function block($instance) {
		extract($instance);
		if($extra_border == 2) {
			echo '<div class="button-container-color">';
		}
		...
		if($url != '') {
			echo '<a ' . $target . ' id="btn-' . $id . '" class="btn btn-default" href="' . esc_url($url) . '">' . esc_attr($text) . '</a>';
		}
		else {
			echo '<a ' . $target . ' id="btn-' . $id . '" class="btn btn-default">' . esc_attr($text) . '</a>';
		}
		...
		echo '<style type="text/css">
				#btn-' . $id . ' { background-color: ' . $background . '; color: ' . $color . '; border-color: ' . $background_hover . '; }
				#btn-' . $id . ':hover { background-color: ' . $background_hover . '; color: ' . $color_hover . '; }
			</style>';
	}
