...
function teo_pagebuilder() {
	if(class_exists('AQ_Page_Builder') ) {
        ...
        require_once('lib/page_builder/filterableportfolio_block.php');
        require_once('lib/page_builder/service_block.php');
        require_once('lib/page_builder/process_block.php');
        require_once('lib/page_builder/timeline_block.php');
        require_once('lib/page_builder/team_block.php');
        require_once('lib/page_builder/portfolio_heading_block.php');
        require_once('lib/page_builder/heading_block.php');
        require_once('lib/page_builder/subheader_block.php');
        require_once('lib/page_builder/button_block.php');
        require_once('lib/page_builder/pricingtable_block.php');
        ...
        aq_register_block('AQ_Timeline_Block');
        aq_register_block('AQ_Team_Block');
        aq_register_block('AQ_PortfolioHeading_Block');
        aq_register_block('AQ_Heading_Block');
        aq_register_block('AQ_Subheader_Block');
        aq_register_block('AQ_CustomButton_Block');
...
if ( !function_exists('teo_scripts') ) {
	function teo_scripts() {
        wp_enqueue_style( 'animsition-css', get_template_directory_uri() . '/css/animsition.min.css', array(), '1.0');
        ...
        wp_enqueue_style( 'contact-form', get_template_directory_uri() . '/css/contact-form.css', array(), '1.0');
        wp_enqueue_style( 'style', get_stylesheet_directory_uri() . '/style.css', array(), '1.0');
        
        $color_scheme = get_theme_mod('teo_color_scheme');
        if($color_scheme == 2) {
            wp_enqueue_style( 'style-cyan', get_stylesheet_directory_uri() . '/css/styles-cyan.css', array(), '1.0');
        }

		wp_enqueue_script( 'jquery');
		wp_enqueue_script( 'jquery.animsition', get_template_directory_uri() . '/js/jquery.animsition.min.js', array('jquery'), '1.0', true);
		wp_enqueue_script( 'jquery-smooth-scroll', get_template_directory_uri() . '/js/smooth-scroll.min.js', array('jquery'), '1.0', true);
		wp_enqueue_script( 'jquery.mixitup', get_template_directory_uri() . '/js/jquery.mixitup.min.js', array(), '1.0', true);
		wp_enqueue_script( 'jquery.pushy', get_template_directory_uri() . '/js/pushy.min.js', array('jquery'), '1.0', true);
		wp_enqueue_script( 'bootstrap', get_template_directory_uri() . '/js/bootstrap.min.js', array('jquery'), '1.0', true);
		wp_enqueue_script( 'scrn2', get_template_directory_uri() . '/js/scrn2.js', array('jquery'), '1.0', true);
		wp_localize_script( 'scrn2', 'MyAjax', array( 'ajaxurl' => admin_url('admin-ajax.php'), 'themeurl' => get_template_directory_uri() ) );
...
class teo_description_walker extends Walker_Nav_Menu
{
...
if( !function_exists( 'teo_show_top_menu') )
{
	function teo_show_top_menu() {
		echo '<ul>';
		$pages = get_pages('number=7&sort_column=menu_order&sort_order=ASC');
		echo '<li><a href="' . get_home_url() . '">' . __('Home', 'teo') . '</a></li>';
...
function teo_register_required_plugins() {
  $plugins = array(
        ...
        array(
            'name'               => 'SCRN II Custom Post Types', // The plugin name.
            'slug'               => 'scrn2-teothemes', // The plugin slug (typically the folder name).
            'source'             => get_template_directory() . '/plugins/scrn2-teothemes.zip', // The plugin source.
        ...
        array(
            'name'               => 'Aqua Page Builder (TeoThemes modified)', // The plugin name.
...
if(! function_exists('teo_customization'))
{
    function teo_customization() {
        $favicon = get_theme_mod('teo_favicon');

        if($favicon != '') {
            $meta_tags = array(
                sprintf( '<link rel="icon" href="%s" sizes="32x32" />', esc_url( $favicon ) ),
                sprintf( '<link rel="icon" href="%s" sizes="192x192" />', esc_url( $favicon ) ),
                sprintf( '<link rel="apple-touch-icon-precomposed" href="%s">', esc_url( $favicon ) ),
                sprintf( '<meta name="msapplication-TileImage" content="%s">', esc_url( $favicon ) ),
            );
